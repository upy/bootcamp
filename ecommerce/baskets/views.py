...
class BasketViewSet(DetailedViewSetMixin, mixins.RetrieveModelMixin,
    ...
    def get_queryset(self):
        ...
        if user_id:
            loggined_basket = queryset.filter(customer_id=user_id,
                                              status=BasketStatus.OPEN).first()
            lookup_url_kwarg = self.lookup_url_kwarg or self.lookup_field
            if lookup_url_kwarg in self.kwargs:
                filter_kwargs = {self.lookup_field: self.kwargs[
                    lookup_url_kwarg]}  # {"slug": "5667880-23423sdfsd"}
                anonymous_basket = queryset.filter(**filter_kwargs).filter(
                    customer__isnull=True).first()
                    ...
                    with atomic():
                        ...
                        for old_basket_item in loggined_basket.basketitem_set.all():
                            new_basket_item = anonymous_basket.basketitem_set.filter(
                                product_id=old_basket_item.product_id).first()
                            ...
                            item_serializer = BasketItemSerializer(
                                ...
                                data={"basket": anonymous_basket.id,
