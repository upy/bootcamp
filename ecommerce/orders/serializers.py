...
from customers.serializers import CustomerSerializer, CitySerializer, AddressSerializer
from orders.models import OrderItem, Order, BillingAddress, ShippingAddress, \
    OrderBankAccount
...
class OrderSerializer(serializers.ModelSerializer):
    class Meta:
        model = Order
        fields = (
            "id", "customer", "basket", "status", "billing_address", "shipping_address",
            "total_price")
...
class BillingAddressSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = (
            "id", "full_name", "line_1", "line_2", "phone", "district", "zipcode",
            "city")
...
class OrderDetailedSerializer(OrderSerializer):
    ...
    class Meta(OrderSerializer.Meta):
        fields = OrderSerializer.Meta.fields + (
            "orderitem_set", "orderbankaccount_set",)
...
class CreateOrderSerializer(serializers.Serializer):
    basket = serializers.SlugRelatedField(slug_field='slug',
    ...
    def create(self, validated_data):
        ...
        shipping_serializer = ShippingAddressSerializer(
            data={"full_name": address_for_shipping.full_name,
        ...
        billing_serializer = BillingAddressSerializer(
            data={"full_name": address_for_billing.full_name,
        ...
        for basket_item in basket.basketitem_set.all():
            for i in range(basket_item.quantity):
                order_item_serializer = OrderItemSerializer(
                    data={"order": order.id, "product": basket_item.product_id,
                          "price": basket_item.price})
                order_item_serializer.is_valid(raise_exception=True)
                order_item_serializer.save()
            stock = basket_item.product.stock
            Stock.objects.filter(id=stock.id).update(quantity=F("quantity")-basket_item.quantity)
        basket.status = BasketStatus.SUBMITTED
        basket.save(update_fields=["status", "modified_at"])
        values = order.orderitem_set.values("order").annotate(total=Sum("price")).values("total")
